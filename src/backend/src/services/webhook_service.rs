...
const N8N_WEBHOOK_URL: &str =
    "https://oversyn.com/webhook-test/6b8bc071-50a4-481a-bce0-2e92bd04b171";
...
pub async fn send_directory_automation(
    chat_id: u64,
    current_path: PathBuf,
    file_infos: Vec<FileInfo>,
) -> String {
    // Create payload with directory information
    let payload = DirectoryAutomationPayload {
        canister_id: ic_cdk::id().to_string(),
        chat_id,
        current_path: current_path.to_string_lossy().to_string(),
        files: file_infos,
    ...
    let json_payload = match serde_json::to_string(&payload) {
        Ok(json) => json,
        Err(e) => return format!("Failed to serialize directory payload: {}", e),
    };

    custom_print!("Sending directory automation payload: {}", json_payload);
    send_payload_to_webhook(json_payload).await
}

// Add this new struct to webhook_service.rs
#[derive(Serialize, Deserialize)]
pub struct DirectoryAutomationPayload {
    pub canister_id: String,
    pub chat_id: u64,
    pub current_path: String,
